<div class="ui grid">
  <div class="four wide column">
    <div class="ui secondary vertical pointing fluid menu">

      <a class="item active" id="inicio" href="/admin">Inicio</a>
      <a class="item" id="estados" href="/admin/estados">Estados</a>
      <a class="item" id="categorias" href="/admin/categorias">Categorias</a>
      <a class="item" id="competencias" href="/admin/competencias">Competencias</a>

      <a class="item" id="recursos" href="/admin/tipo_recursos">Tipo de Recursos</a>
      <a class="item" id="recursos" href="/admin/recursos">Recursos</a>



<a class="item" id="publicaciones" href="/admin/publicaciones">Publicaciones</a>

      <a class="item" id="comentarios" href="/admin/comentarios">Comentarios</a>
      <a class="item" id="tipo_evaluacion" href="/admin/tipo_evaluacion">Tipos de evaluacion</a>
      <a class="item" id="usuarios" >Usuarios</a>
      <a class="item">Estadisticas</a>
      <a class="item">Otros</a>

    </div>
  </div>
